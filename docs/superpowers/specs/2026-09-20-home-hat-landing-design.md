...
- Interaction: slow auto-rotate; drag to rotate; clicking the hat advances the
  carousel by one item.
- Fallback (`HatFallback`): static SVG cone, shown while the scene loads, when
  WebGL is unavailable, and when `prefers-reduced-motion` is set (no auto-rotate).
...
- Palette: cream background, navy, brass gold. Serif display font via
  `next/font`. Colour/font tokens are scoped to the Home page so other pages are unaffected.
...
| Add | `components/home/ArchCarousel.tsx` — carousel + hat coupling state |
| Add | `components/home/sectionIcons.tsx` — inline SVG icons |
| Edit | `app/page.tsx` — new layout |
| Edit | `lib/i18n/dictionaries/en.json`, `vi.json` — new strings |
| Edit | `package.json` — add `three`, `@react-three/fiber` (and `@types/three`) |
...
The project has no test framework. Verification is `npm run build`,
`npm run lint`, and running the dev server to check desktop and mobile widths:
